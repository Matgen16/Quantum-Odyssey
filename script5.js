...
function drawOneWayCollisions() {
    ctx.fillStyle = "#33dd99";
    collisionsOneWay.forEach((wall) => {
        ctx.fillRect(wall.x, wall.y, wall.width, wall.height);
    });
}
...
function drawCollisionsIn1() {
    ...
    ctx.fillRect(0, 195, window.innerWidth / 2 - 50, 30)
    ctx.fillRect(window.innerWidth / 2 + 50, 195, window.innerWidth / 2, 30)
}
...
function mouseMove(e) {
    if (isDragging && quantumState === 1) {
        for (let i = 0; i < collisionsIn1.length; i++) {
            if (player.x > window.innerWidth / 2 - 50 && player.x + player.width < window.innerWidth / 2 + 50) {
